...
solveModel1_GE_sim=function(param,data,settings){
  # Settings
  outerdiff = Inf;    
  w = w_init;
  u = u_init;
  ...
  while(outerdiff>tol & iter < 10){
    ...
    # 5) Number of firms by location
    num_firms = number_firms(beta1, beta_tilde_sector, w, L_j_eff, sigma1, q)
    beta_tilde=num_firms$beta_tilde
    beta_factor=num_firms$beta_factor
    M=num_firms$M
    sigma_cons=num_firms$sigma_cons
    ...
    P = agg_price_ind$P
    P_s = agg_price_ind$P_s
    sigma1_tr = agg_price_ind$sigma1_tr
    ...
    if(settings$endo_Lr==1){
      lambda_i = array_operator(array_operator(u, U_i^eta1, '*'), sumDims(array_operator(u, U_i^eta1, '*'),1), '/')
    }

    outerdiff = max(w_diff)
    iter = iter + 1;
